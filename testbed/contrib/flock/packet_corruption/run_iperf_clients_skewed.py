...
parser.add_argument("hosts_rack_file", help="A file containing list of host ips running iperf3 servers from certain ports")
parser.add_argument("busy_hosts_file", help="A file containing list of host ips of busy hosts")
...
if(args.iperf_role == "servers"):
    for port in iperf_server_ports + [same_rack_port]:
    ...
    host_rack_map = dict()
    hosts = []
    with open(args.hosts_rack_file) as f:
        for line in f.readlines():
            line = line.strip()
            if line != "":
                host, rack = line.split()
            if host != args.collector_ip:
                host_rack_map[host] = rack
                if host != args.host_ip:
                    hosts.append(host)
    ...
    busy_hosts = []
    busy_self = False
    with open(args.busy_hosts_file) as f:
        for line in f.readlines():
            busy_host = line.strip()
            if busy_host != "" and busy_host != args.collector_ip:
                assert (busy_host in hosts or busy_host == args.host_ip)
                if busy_host == args.host_ip:
                    busy_self = True
                else:
                    busy_hosts.append(busy_host)

    idle_hosts = list(set(hosts) - set(busy_hosts))
    ...
    #assume same distribution as simulations
    remaining_flows = 1000000
    interflow_sec = 0.10 #0.30 #0.20 #0.10 #0.020
    if busy_self:
        interflow_sec = interflow_sec / 19

    connection_threads = []
    while(remaining_flows > 0):
        time.sleep(interflow_sec)
        remaining_flows -= 1
        dest_ip = random.choice(idle_hosts)
        if random.randint(0, 99) < 5:
            dest_ip = random.choice(busy_hosts)
        dest_port = random.choice(iperf_server_ports)
        # if same rack, use a different port to avoid tc queue
        if (host_rack_map[dest_ip] == host_rack_map[args.host_ip]):
            dest_port = same_rack_port
